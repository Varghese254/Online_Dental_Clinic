...
if(isset($_POST["btnreset"]))
{
	...
	{
		$sela="select * from tbl_admin where admin_email='".$_SESSION["lgid"]."'";
		$rowa=$con->query($sela);
	
		$sell="select * from tbl_dentist where dentist_email='".$_SESSION["lgid"]."'";
		$rowl=$con->query($sell);
	
		$selu="select * from tbl_user where user_email='".$_SESSION["lgid"]."'";
		$rowu=$con->query($selu);
	}
		if($dataa=$rowa->fetch_assoc())
		{
			$up="update tbl_admin set admin_password='".$_POST["txtnew"]."' where admin_email='".$_SESSION["lgid"]."'";
			$con->query($up);
		}
		else if($datal=$rowl->fetch_assoc())
		{
			$up="update tbl_dentist set dentist_pass='".$_POST["txtnew"]."' where dentist_email='".$_SESSION["lgid"]."'";
			$conn->query($up);
		}
		else if($datau=$rowu->fetch_assoc())
		{
			$up="update tbl_user set user_pass='".$_POST["txtnew"]."' where user_email='".$_SESSION["lgid"]."'";
			$con->query($up);
		?>
        <script>
